...
class ChatPageState extends State<ChatPage> {
  ...
  _imListener(){
    print('！！！！！！接受消息！！！！！');
     RongIMClient.onMessageReceived = (Message msg,int left) {
      ...
      print("receive message Content:"+msg.content.conversationDigest());
    };
  }
  ...
  _getHistoryChat() async {
    ...
    api.getData(context, 'history_message',formData: formData).then((val){
        ...
        // 避免历史记录和新来的消息冲突，首先清空一遍数据，再赋值（只有首次进入页面才会清空，否则每次下拉刷新都会清空）
        if(!firstBol){
                          ...
                          children: <Widget>[
                            ...
                            InkWell(
                              onTap: () async{
                                ...
                                txtMessage.content = _textEditingController.text;
                                Message msg = await RongIMClient.sendMessage(RCConversationType.Private,arguments['recv_id'].toString(), txtMessage);
                                print("消息发送成功：接收消息方id = "+msg.senderUserId);
                                // Map contentArguments = {
                                //   'type':'private_chat',
                                //   'content_type':'text',
  ...
  chatHistory(response, gl_context, isMe){
    switch (response['content_type']) {
      case 'text':
          Provide.value<SocketProvider>(gl_context).setRecords(response['content'], 'text',  isMe, isNetWork: true, history: true);
        break;
      case 'img':
          Provide.value<SocketProvider>(gl_context).setRecords(response['content'],'img', isMe, isNetWork: true, history: true);
        break;
      case 'video':
          Provide.value<SocketProvider>(gl_context).setRecords(response['content'],'video',  isMe, isNetWork: true, history: true);      
        break;
      case 'audio':
          Provide.value<SocketProvider>(gl_context).setRecords(response['content'],'audio', isMe, time_length:response['time_length'], isNetWork: true, history: true);
        break;
      default:
    }
  }
