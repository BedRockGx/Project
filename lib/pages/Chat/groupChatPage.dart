...
class GroupChatPage extends StatefulWidget {
  final arguments;
  GroupChatPage(this.arguments);
  @override
  GroupChatPageState createState() {
    return GroupChatPageState(this.arguments);
  }
}

class GroupChatPageState extends State<GroupChatPage> {
  final arguments;
  GroupChatPageState(this.arguments);
  ...
  void _sendMsg(String msg) {
    setState(() {
      showSend = false;
      Provide.value<SocketProvider>(context).setGroupRecords(msg,'text', true);
    });
  ...
  stopRecord(String path,double audioTimeLength ) async {
    ...
    api.postData(context, 'uploadFile', formData: await FormData1(path)).then((data){
      ...
      Provide.value<SocketProvider>(context).setGroupRecords(path,'audio', true, time_length:audioTimeLength.toString());
  ...
  _getHistoryChat() async {
    ...
    print(formData);
    api.getData(context, 'viper_history_message',formData: formData).then((val){
      if(val.toString() == '[]'){
        print('到顶了！！！！');
        Fluttertoast.showToast(msg: '已经到顶了~');
        return;
      }
      ...
      for(var item in response){
        ...
        print(item['content']);
        if(item['is_me'] == 1){
          chatHistory(json.decode(item['content'].toString()), item, context, true);
        }else{
          chatHistory(json.decode(item['content'].toString()), item, context, false);
        }
      }
    }
  );
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        actions: <Widget>[
          arguments['isVip'] ? 
          IconButton(
            icon: Icon(IconData(0xe624, fontFamily: 'myIcon')),
            onPressed: () {
              print(arguments);
              // Fluttertoast.showToast(msg: '功能没有完善有');
              Navigator.pushNamed(context, '/vipList');
            },
          ):
          Container(
            child: Text(''),
                        ...
                        slivers: <Widget>[
                          SliverList(
                            delegate: SliverChildBuilderDelegate(
                              (context, index) {
                                return _buildMsg(val.grouprecords[index]);
                              },
                              childCount: val.grouprecords.length,
                            ),
                          ),
                        ],
                        onLoad: (){
                             ...
                             onPressed: () async {
                               ...
                               if(img_url!=null){
                                ...
                                api.postData(context, 'uploadFile', formData: await FormData1(img_url.path)).then((data){
                                  ...
                                  print(contentArguments);
                                  Provide.value<SocketProvider>(context).setGroupRecords(localImage,'img', true);
                                  // socket发送消息
                                  val.socket.write(json.encode(contentArguments));
                                });
                              }
                             ...
                             onPressed: () async {
                               ...
                               if(video_url!=null){
                                ...
                                api.postData(context, 'uploadFile', formData: await FormData1(video_url.path)).then((data){
                                  ...
                                  print(contentArguments);
                                  Provide.value<SocketProvider>(context).setGroupRecords(localVideo.path,'video', true);
                                  // socket发送消息
                                  val.socket.write(json.encode(contentArguments));
                                });
                              }
  ...
  // 构建消息视图
  Widget _buildMsg(GroupChatRecord entity) {
          ...
          children: <Widget>[
            Card(
              ...
              child: Container(
                height: 40.0,
                width: 40.0,
                child: Image.network('$base_url${entity.head_pic}', fit: BoxFit.cover,),
              ),
            ),
            Column(
              crossAxisAlignment: CrossAxisAlignment.start,
  ...
  // 判断历史记录的类型存储到数组中
  chatHistory(response, historyContent, gl_context, isMe){
    switch (response['content_type']) {
      case 'text':
          Provide.value<SocketProvider>(gl_context).setGroupRecords(response['content'], 'text',  isMe, isNetWork: true, head_pic: historyContent['head_pic'], nickname: historyContent['nickname'], history: true);
        break;
      case 'img':
          Provide.value<SocketProvider>(gl_context).setGroupRecords(response['content'],'img', isMe, isNetWork: true, head_pic: historyContent['head_pic'], nickname: historyContent['nickname'], history: true);
        break;
      case 'video':
          Provide.value<SocketProvider>(gl_context).setGroupRecords(response['content'],'video',  isMe, isNetWork: true, head_pic: historyContent['head_pic'], nickname: historyContent['nickname'], history: true);      
        break;
      case 'audio':
          Provide.value<SocketProvider>(gl_context).setGroupRecords(response['content'],'audio', isMe, time_length:response['time_length'], isNetWork: true, head_pic: historyContent['head_pic'], nickname: historyContent['nickname'], history: true);
        break;
      default:
    }
  }
